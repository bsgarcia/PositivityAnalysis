...
try
    data = load('data/fit_exp/online_exp_no_rel');
    lpp = data.data('lpp');
    parameters = data.data('parameters');  %% Optimization parameters 
    ll = data.data('ll');
    hessian = data.data('hessian');
catch
    [parameters, lpp, ll, hessian] = runfit(...
        subjecttot, nparam, nmodel, whichmodel, con, cho, out);
end
...
% % --------------------------------------------------------------------
figure
params = {[2], [2, 3], [2, 3], [2, 4], [2, 5], [2, 6, 7]};
...
function [parameters, lpp, ll, hessian] = runfit(...
    subjecttot, nparam, nmodel, whichmodel, con, cho, out)
    ...
    for nsub = 1:subjecttot
        ...
        for model = whichmodel
            [
                ...
                @(x) getlppnorel(x, con{nsub}, cho{nsub}, out{nsub}, model),...
                [1, .5, .5, 0, 0, .5, .15, .15],...
                [], [], [], [],...
                [0, 0, 0, -2, -2, 0, 0, 0],...
                [Inf, 1, 1, 2, 2, 1, 1, 1],...
            ...
            [
                ...
                @(x) getllnorel(x, con{nsub}, cho{nsub}, out{nsub}, model),...
                [1, .5, .5, 0, 0, .5, .15, .15],...
                [], [], [], [],...
                [0, 0, 0, -2, -2, 0, 0, 0],...
                [Inf, 1, 1, 2, 2, 1, 1, 1],...
    ...
    save('data/fit_exp/online_exp_no_rel', 'data');
    close(w);
...
% end
% 
% disp(sprintf('N subjects %d', length(best_ids)));
% 
% %% Define variables
% % --------------------------------------------------------------------
% whichmodel = [1, 2, 3, 4, 5];
...
% models = {'RW', 'RW\pm',...
%     'RW\pm_{\omega^-}', 'RW_\omega', 'RW_\phi',...
%     'Gradual Perseveration', 'Full', 'Bayesian'};
% 
% paramlabels = {
%     '\beta', '\alpha+', '\alpha-', '\omega', '\phi',...
%     '\tau', '\sigma_{xi}', '\sigma_{\epsilon}'}; 
...
%         end
%         %%Save the data
%         data = containers.Map({'parameters', 'lpp' 'll'}, {parameters, lpp, ll});
...
% %parametersfitbarplot(parameters, nmodel, whichmodel, models);
% %Bar plots
% parameters = parameters(best_ids, :, :);
% figure
% subplot(2, 4, 1)
% barplot_param_comparison(parameters, [2, 3], 2, paramlabels, models, 0.5);
% subplot(2, 4, 4)
% barplot_param_comparison(parameters, [2, 5], 5, paramlabels, models, 0.5);
% subplot(2, 4, 3)
% barplot_param_comparison(parameters, [2, 4], 4, paramlabels, models, 0.5);
% subplot(2, 4, 2)
% barplot_param_comparison(parameters, [2, 3], 3, paramlabels, models, 0.5);
% 
% bias1 = (parameters(:, 2, 2) - parameters(:, 3, 2))./...
%     (parameters(:, 2, 2) + parameters(:, 3, 2)) ;
% bias2 = (parameters(:, 2, 3) - parameters(:, 3, 3)) ./...
...
%             case 6
%                 y(model, 1:nfpm(model), 1) = mean(parameters(:, [1, 2, 5, 6], model), 1);
%                 s(model, 1:nfpm(model), 1) = sem(parameters(:, [1, 2, 5, 6], model));
%             case 8 
%                 y(model, 1:nfpm(model), 1) = mean(parameters(:, [1, 7, 8], model), 1);
%                 s(model, 1:nfpm(model), 1) = sem(parameters(:, [1, 7, 8], model));
%         end
%     end
%     figure
%     hBar = bar(y, 'FaceAlpha', 0.7);
...
% function barplot_param_comparison(parameters, param_idx, model_idx, labels, models, ymax)
%     %hold on
%     nsub = length(parameters(:, 1, 1));
%     y1 = parameters(:, param_idx(1), model_idx);
%     y2 = parameters(:, param_idx(2), model_idx);
%     if length(param_idx) == 3
%         means = [mean(y1), mean(y2), -1];
%         errors = [sem(y1), sem(y2), 0];
%     else
%         means = [mean(y1) mean(y2)];
%         errors = [sem(y1) sem(y2)];
%     end
%     b = bar(means, 'EdgeColor', 'black');
%     box off
%     hold on
