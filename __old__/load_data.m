...
% Load data function
function [con, con2, cho, out, nsubs, p, correct, qvalues] = load_data(type, cond)
    ...
    switch type
        ...
        case {'sim2', 'sim2sessions', 'sim_2'}
            if ~exist('cond', 'var')
                error('You should specify a condition');
            end
                
            switch cond
                case {'risk_new', 'risk', 'statusquo1', 'statusquo2'}
                    data = load(sprintf('data_sim_2sessions/%s', cond));
                    data = data.data;
                    nsubs = length(data);
                    for i = 1:nsubs
                        con{i} = data{i}(:, 3, :);
