...
function plotheatmap(data, xlabels, ylabels, xaxislabel, yaxislabel, titlelabel,...
    ...
    saveas(f, sprintf('2sessions_recovery_%s_%d.png', titlelabel, length(xlabels)));
end
...
function [lpp, parameters] = getdata(file)
        data = load(sprintf('data/fit_sim_2sessions/%s_new', file));
        lpp = data.data('lpp');
...
function [lpp, parameters] = runfit(file, nmodel, whichmodel, nparam,...
    ...
    [
        ...
        out, ...
        nsubs, ...
    ] = load_data('sim2', sprintf('%s_new', file));
    ...
    save(sprintf('data/fit_sim_2sessions/%s', file), 'data');
end
